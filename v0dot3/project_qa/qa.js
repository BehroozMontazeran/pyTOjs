...
for (let i = 1; i <= 10; i++) {
    ...
    // Encode abstracts
    const encoded_abstracts_part1 = encode_abstracts_sliding_window(df_part1['Combined_Info']);

    // Save encoded abstracts
    save_encoded_abstracts(encoded_abstracts_part1, `encoded_data_part_${i}.pt`);
}

// Load and concatenate encoded abstracts from all parts
const encoded_abstracts_parts = [];
for (let i = 1; i <= 10; i++) {
    const encoded_abstracts_part = load_encoded_abstracts(`encoded_data_part_${i}.pt`);
    encoded_abstracts_parts.push(encoded_abstracts_part);
}
