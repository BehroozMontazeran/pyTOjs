class WorldArgs {
    constructor(no_gui, fps, turn_based, update_interval, save_replay, replay, make_video, continue_without_training, log_dir, save_stats, match_name, seed, silence_errors, scenario) {
        this.no_gui = no_gui;
        this.fps = fps;
        ...
        this.replay = replay;
        this.make_video = make_video;
        this.continue_without_training = continue_without_training;
        this.log_dir = log_dir;
        this.save_stats = save_stats;
        this.match_name = match_name;
        this.seed = seed;
        this.silence_errors = silence_errors;
        this.scenario = scenario;
    }
}class Trophy {
    constructor() {
        this.coin_trophy = null;
        this.suicide_trophy = null;
        this.time_trophy = null;
    }
}class GenericWorld {
    constructor(args) {
        this.args = args;
        this.colors = s.AGENT_COLORS.slice();
        this.round = 0;
    ...
    new_round() {
        ...
        this.replay = {
            'round': new_round,
            'arena': np.array(this.arena),
            'coins': this.coins.map(c => c.get_state()),
            'agents': this.active_agents.map(a => a.get_state()),
            'actions': {},
            'permutations': []
        };
    ...
    add_agent(agent_dir, name, train = false) {
    ...
    perform_agent_action(agent, action) {
        if (action === 'UP' && this.tile_is_free(agent.x, agent.y - 1)) {
            ...
            this.bombs.push(new Bomb([
                agent.x,
                agent.y
            ], agent, BOMB_TIMER, BOMB_POWER, agent.bomb_sprite));
                ...
                for (const a of this.active_agents) {
                    if (!a.dead && explosion.blast_coords.has(`${ a.x },${ a.y }`)) {
                        ...
                        if (a === explosion.owner) {
                            ...
                            explosion.owner.trophies.push(pygame.transform.smoothscale(a.avatar, [
                                15,
                                15
                            ]));
                        }
    ...
    time_to_stop() {
        ...
        if (this.active_agents.length === 1 && this.arena.every(row => row.every(cell => cell === 0)) && this.coins.every(coin => !coin.collectable) && this.bombs.length + this.explosions.length === 0) {
            this.logger.info('One agent left alive with nothing to do, wrap up round');
            return true;
        }
        if (this.agents.some(a => a.train) && !this.args.continue_without_training && !this.active_agents.some(a => a.train)) {
    ...
    constructor(world) {
        this.world = world;
        this.screenshot_dir = s.ASSET_DIR + '/screenshots';
    ...
    render_text(self, text, x, y, color, halign = 'left', valign = 'top', size = 'medium', aa = false) {
    ...
    render() {
        ...
        for (let x = 0; x < this.world.arena.shape[1]; x++) {
            for (let y = 0; y < this.world.arena.shape[0]; y++) {
                if (this.world.arena.get([
                        x,
                        y
                    ]) === -1) {
                    this.screen.blit(this.t_wall, [
        ...
        this.render_text(`Step ${ this.world.step }`, s.GRID_OFFSET[0], s.HEIGHT - s.GRID_OFFSET[1] / 2, [
            64,
            64,
            64
        ], {
        ...
        for (let i = 0; i < agents.length; i++) {
            ...
            this.render_text(agents[i].display_name, 650, y_base + 50 * i, agents[i].dead ? [
                64,
                64,
                64
            ] : [
            ...
            for (let j = 0; j < agents[i].trophies.length; j++) {
                this.screen.blit(agents[i].trophies[j], [
                    660 + 10 * j,
                    y_base + 50 * i + 12
                ]);
            }
            this.render_text(`${ agents[i].score }`, 830, y_base + 50 * i, [
                255,
                255,
                255
            ], {
            ...
            this.render_text(`${ agents[i].total_score }`, 890, y_base + 50 * i, [
                64,
                64,
                64
            ], {
        ...
        if (!this.world.running) {
            ...
            this.render_text(leading.display_name, x_center, 320, color, {
                valign: 'top',
                halign: 'center',
                size: 'huge'
            });
            this.render_text('has won the round!', x_center, 350, color, {
                valign: 'top',
                halign: 'center',
                size: 'big'
            });
            ...
            if (leading_total === leading) {
                this.render_text(`${ leading_total.display_name } is also in the lead.`, x_center, 390, [
                    128,
                    128,
                    128
                ], {
                ...
                this.render_text(`But ${ leading_total.display_name } is in the lead.`, x_center, 390, [
                    128,
                    128,
                    128
                ], {
    ...
    make_video() {
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            subprocess.call([
                ...
                'glob',
                '-i',
                this.screenshot_dir + '/' + this.world.round_id + '_*.png'
            ].concat(PARAMS[video_file.split('.').pop()], video_file));
        }
