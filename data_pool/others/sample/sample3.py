...
class MarkowitzPortfolio(object):
	...
	def downloadData(self,dateBegin,dateEnd,stockName,timeMarker): #format for the dates: "month/day/year" stockName ex : "MSFT" for Microsoft
		...
		if (timeMarker=='monthly'):
			...
			timeMarker='d'
		dataFile = urllib.request.urlopen("http://ichart.finance.yahoo.com/table.csv?s="+stockName+"&a="+dateBeginTab[0]+"&b="+dateBeginTab[1]+"&c="+dateBeginTab[2]+"&d="+dateEndTab[0]+"&e="+dateEndTab[1]+"&f="+dateEndTab[2]+"&g="+timeMarker+"&ignore=.csv")
		temp=dataFile.read()
		temp=temp.split("\n")
		header=temp.pop(0)
